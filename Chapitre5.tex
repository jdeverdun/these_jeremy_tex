...
qu'ils proposent pour comprendre l'autorégulation du système. Enfin, nous présenterons en détail notre propre modèle dynamique, qui intègre les deux approches « compartiment » et « modélisation spécifique », et exposerons ses résultats provisoires, avant de parler des perspectives de
...
\caption{Schéma du modèle d'Ursino (\cite{Ursino1997}). La figure A représente un point de vue électrique, la B un point de vue mécanique. $q$ représente le débit sanguin cérébral, $P$ les pression, $R$ les résistances, $C$ les capacités et $I$ le taux d'injection fictif de LCS.}
...
\caption{Schéma du modèle de Sorek (\cite{Sorek1988}). $R$ représente les résitances, $C$ les compliances, $[ ]$ les pressions (en mmHg), $( )$ les flux (en ml/min) et $< >$ le volume (en ml).}
\label{fig:4_2_sorek}	
\end{figure}
Les paramètres de
...
\end{equation}
où $\mu$ est le coefficient de viscosité dynamique (en Pa . s), $S$ l'aire totale ($S = NA$, $N$ étant le nombre de vaisseaux). Notons qu'ici F désigne la friction et non le flux. Cette loi de Poiseuille permet d'associer au modèle les paramètres structuraux des vaisseaux, soit tirés
...
\caption{Schéma du modèle de Linninger (\cite{Linninger2009}). Avec cAr la carotide interne, Ar les artères, Al les artérioles, Cp les capillaires, Vl les veinules, V les veines, Lv les ventricules latéraux, 3V le troisième ventricule, 4V le quatrième ventricule, SAS l'espace sous arachnoïdien, vSinus le sinus veineux et JV les veines jugulaires.}
...
\afterpage{%
	...
	\begin{landscape}
	\begin {table}
	\captionof{table}{Tableau résumant les données utilisées dans les différents modèles de type compartiments de la littérature. Données théoriques indiquent des valeurs de la littérature, des données estimées des valeurs basées sur la littérature puis ajustées, et enfin mesurées, des valeurs issues de mesures réelles (IRM etc.).} 
	\label{tab:modeles} 
	\centering
...
\begin{figure}[!t]
\centering
\includegraphics[width=11cm]{augmentation_veine}
...
\begin{figure}[!t]
\centering
\includegraphics[width=11cm]{augmentation_arteres}
